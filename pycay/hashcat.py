...
class HC_STATE:
        ...
        def __str__(self):
            ...
            string += '\nProgress'.ljust(18) + ": " + "{:d}/{:d}  {:.3f}%".format(self.progress[0], self.progress[1], self.get_progress())
...
class Hashcat(threading.Thread):
    ...
    reserved_keywords = ('STATUS', 'SPEED', 'EXEC_RUNTIME', 'CURKU', 'PROGRESS', 'RECHASH', 'RECSALT', 'TEMP','REJECTED')
    ...
    def __init__(self, exec_path: str ="./hashcat/hashcat64.bin", hash_mode: int = 0, attack_mode: int = 0,
        ...
        self.external_commands = ['--quiet',  "--status", "--remove", "--machine-readable", "--restore-disable",
                                  "--potfile-disable"]  # Kullanıcının parametresine ek olarak yürülecek parametreler
    ...
    def cmd(self, command: str):
        try:
            ...
            if command == 'q' or command == '-q':
                threading.Thread(target=(lambda: (self.get_hc_state()))).start()
